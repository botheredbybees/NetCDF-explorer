//For exploring CSIRO CDF files
//Uses:
// the wxWidgets GUI library www.wxwidgets.org
// Unidata's NetCDF library www.unidata.ucar.edu/software/netcdf/
// code snippets written by Pedro Vicente github.com/pedro-vicente
...
//OPeNDAP
//see http://www.cmar.csiro.au/thredds/catalog.html for some urls
...
wxString GetAppName()
{
  wxString str(wxT("CSIRO CDF Explorer"));
  return str;
}
...
void wxTreeCtrlExplorer::LoadItem(wxItemData *item_data)
{
  ...
  for (int idx_dmn = 0; idx_dmn < nbr_dmn; idx_dmn++)
  {
    ...
    {
      wxString var_nm(item_data->m_item_data_prn->m_var_nms[idx_var]);

      if (var_nm == wxString(dmn_nm_var))
      {
        has_crd_var = 1;
        break;
      }
